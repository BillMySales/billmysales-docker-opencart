#!/bin/bash
# Backups of the database and the store files.
#
#   backup.sh            # loop: back up now, then every BACKUP_INTERVAL_HOURS
#   backup.sh now        # one backup
#   backup.sh list       # list backups
#   backup.sh restore <timestamp>   # restore DB and store files from a backup
...
# (code, images, extensions and storage, without caches and sessions),
...
backup() {
    ...
    tar -C "${WWW_DIR}" --exclude='./storage/cache/*' --exclude='./storage/session/*' \
        -czf "${tmp}-files.tar.gz" ./html ./storage
...
restore() {
    ...
    echo "==> Restoring store files from ${files}"
    find "${WWW_DIR}/html" "${WWW_DIR}/storage" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
